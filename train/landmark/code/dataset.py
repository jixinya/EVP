...
class SMED_1D_lstm_landmark_pca(data.Dataset):
    def __init__(self,
        ...
        self.num_frames = 16
        self.data_root = '/home/thea/data/MEAD/ATnet_emotion/dataset/'
        self.audio_root = '/home/thea/data/MEAD/ATnet_emotion/dataset/MFCC'
        self.train = train
        
        file = open('/home/thea/data/MEAD/ATnet_emotion/dataset/train_list.pkl', "rb") #'rb'-read binary file
        self.train_data = pickle.load(file)
        file.close()
        
        file = open('/home/thea/data/MEAD/ATnet_emotion/dataset/test_list.pkl', "rb") #'rb'-read binary file
        self.test_data = pickle.load(file)
        file.close()
        
        self.pca = torch.FloatTensor(np.load('/home/thea/data/MEAD/ATnet_emotion/basics/U_106.npy')[:, :16])
        self.mean = torch.FloatTensor(np.load('/home/thea/data/MEAD/ATnet_emotion/basics/mean_106.npy'))
    ...
    def __getitem__(self, index):
        if self.train == 'train':
            ...
            lmark_path = os.path.join(self.data_root, 'train', data_folder)
            audio_path = os.path.join(self.audio_root, data_folder)
        ...
        if self.train == 'test':
            ...
            lmark_path = os.path.join(self.data_root,'test', data_folder)
            audio_path = os.path.join(self.audio_root, data_folder)
            lmark = np.load(lmark_path)
            mfcc = np.load(audio_path)
